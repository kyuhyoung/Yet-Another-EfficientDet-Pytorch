...
class CustomDataset(object):
    def __init__(self, root, transforms, is_xywh, phase='train'):
        
        self.is_xywh = is_xywh
        self.root = os.path.join(root, phase)
        self.transforms = transforms
        self.imgs = sorted(list(glob(self.root + '/*jpg') + list(glob(self.root + '/*png'))))
        tree = elemTree.parse(os.path.join(self.root, phase + '.xml'))
        root = tree.getroot()
        self.categories = root.find('meta').find('task').find('labels').findall('label')
        #self.categories = t0.findall('label')
        #t1 = elemTree.parse(os.path.join(self.root, phase + '.xml')).findall('label');
        #print('self.categories : ', self.categories); 
        #print('type(self.categories) : ', type(self.categories)); 
        #print('t1 : ', t1); #exit(0)
        self.labels = sorted(root.findall('image'), key=lambda x: x.attrib['name'])
...
def data_loader(root, batch_size, is_xywh, phase='train') :
    is_train = False
    shuffle = False
    if phase == 'train' :
        is_train = True
        shuffle = True
    dataset = CustomDataset(root, get_transform(train=is_train), is_xywh, phase=phase)
