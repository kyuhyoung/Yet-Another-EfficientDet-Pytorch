...
def main():
    ...
    is_xywh = False    
    if mode == 'train':
        opt = train.get_args(sys.argv)
        dataset_train = CustomDataset(DATASET_PATH, get_transform(True), is_xywh, 'train')
        dataset_val = CustomDataset(DATASET_PATH, get_transform(False), is_xywh, 'val')
        train.train(opt, dataset_train, dataset_val)
        '''        
